...
;(function ($, document, window, navigator, undefined) {
    ...
    IonRangeSlider.prototype = {
        ...
        drawLabels: function () {
            ...
            // MOMCHIL Momchil startuplinx TODO FIXME blah HACK hack
            var from_hack = this.result.from;
            var to_hack = this.result.to;
            if (this.options.transform) {
                from_hack = this.options.transform(from_hack, this.result.min, this.result.max);
                to_hack = this.options.transform(to_hack, this.result.min, this.result.max);
            }
            ...
            if (this.options.type === "single") {
                ...
                if (values_num) {
                    ...
                    if (this.options.decorate_both) {
                        text_single = this.decorate(this._prettify(from_hack));
                        text_single += this.options.values_separator;
                        text_single += this.decorate(this._prettify(to_hack));
                    } else {
                        text_single = this.decorate(this._prettify(from_hack) + this.options.values_separator + this._prettify(to_hack), from_hack);
                    }
                    text_from = this.decorate(this._prettify(from_hack), from_hack);
                    text_to = this.decorate(this._prettify(to_hack), to_hack);
